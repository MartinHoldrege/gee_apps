/********************************************************
 * Purpose:
 * Simple app to show basic layers shown in Noel et al 2025 (https://doi.org/10.1016/j.foreco.2025.123075)
 * showing pinyon juniper community suitability and related metrics. 
 * 
 * Script Started: Septer 22, 2025
 * 
...
var usageAppName = 'futurepjcom'; // unique name for this app, (pasted into tag name used by cloud metrics)
// dependencies -----------------------------------------------------------
...
var load = require("users/MartinHoldrege/gee_apps:scripts/01_pjcom_load-layers.js");
var descript = require("users/MartinHoldrege/gee_apps:scripts/01_pjcom_description.js");
...
var updateLeftMap = function(mapToChange) {
  var lyr = load.loadLayer(selectD.varLeft, selectD.scenLeft);
  mapToChange.layers().set(indexMain, lyr);
};

var updateRightMap = function(mapToChange) {
  var lyr = load.loadLayer(selectD.varRight, selectD.scenRight);
  mapToChange.layers().set(indexMain, lyr);
};
...
var addSelectors = function (mapToChange, side, updateFun, position) {
    ...
    var labelVar2 = ui.Label("(*denotes variable only available for future time-periods)", 
      {fontSize: '11px',
    ...
    var controlPanel =
        ui.Panel({
            widgets: [labelVar, labelVar2, selectVar, labelScen, selectScen],
            style: {
                position: position,
                padding: '2px',
                margin: '2px'
...
leftMap.centerObject(ee.Image(load.path + 'pj_overlay_fullwest_numeric_layers'), 5); // update as needed
